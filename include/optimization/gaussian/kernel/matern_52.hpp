...
namespace optimization::kernel {
    ...
    public:
        ...
        T compute(const Eigen::Matrix<T, Eigen::Dynamic, 1> &x,
            ...
            return variance_ * (1.0 + z + z * z / 3.0) * std::exp(std::max(-z, log_min_)) +
                ...
                for (int j = i; j < n; ++j) {
                    ...
                    switch (param_index) {
                        ...
                        case 1: // Gradient w.r.t. variance
                            K_grad(i, j) = (1.0 + z + z * z / 3.0) * exp_term;
                            break;
...
namespace optimization::kernel {
    ...
    public:
        ...
        T compute(const Eigen::Matrix<T, Eigen::Dynamic, 1> &x,
            ...
            const T r = (x - y).norm();
            const T z = sqrt_5_ * r / length_scale_;
            // Use exp(log(...)) to prevent underflow and add epsilon for numerical stability
            return variance_ * (1.0 + z + z * z / 3.0) * std::exp(std::max(-z, log_min_)) +
                   std::numeric_limits<T>::epsilon();
        }
...
#pragma omp parallel for
        ...
        // Update the kernel parameters with bounds checking and adaptive regularization
        void setParameters(T length_scale, T variance, T noise_variance) {
            T eps = std::numeric_limits<T>::epsilon();
            T old_length_scale = length_scale_;
            T old_variance = variance_;
    ...
    private:
        ...
        T noise_variance_; // Noise variance (jitter for numerical stability)
        static constexpr T sqrt_5_ = 2.236067977499790; // Precomputed sqrt(5)
        static constexpr T log_min_ = std::log(std::numeric_limits<T>::min());
