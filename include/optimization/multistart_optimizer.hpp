...
namespace optimization {
    ...
    public:
        ...
        explicit MultiStartViewpointOptimizer(const Config &config) : config_(config), rng_(std::random_device{}()) {}

        [[nodiscard]] std::optional<ViewPoint<T>>
        optimize(const Image<> &target, const std::shared_ptr<processing::image::ImageComparator> &comparator) {
            ...
            for (int restart = 0; restart < config_.max_restarts; ++restart) {
                ...
                try {
                    ...
                    if (best_viewpoint && best_score > global_best_score) {
                        ...
                        LOG_INFO("New global best viewpoint found: {}, Score: {}", best_viewpoint->toString(),
                                 best_score);
                    }
                    ...
                    LOG_ERROR("Error during optimization attempt {}: {}. Continuing with next attempt.", restart + 1,
                              e.what());
                }
            }
    ...
    private:
        ...
        [[nodiscard]] std::pair<std::optional<ViewPoint<T>>, T>
        runSingleOptimization(const Image<> &target,
            ...
            auto gpr = std::make_unique<optimization::GPR<optimization::kernel::Matern52<T>>>(*kernel);

            auto cache = std::make_unique<cache::ViewpointCache<T>>(typename cache::ViewpointCache<T>::CacheConfig{});
            auto acquisition =
                    std::make_unique<optimization::Acquisition<T>>(typename optimization::Acquisition<T>::Config{});
            auto sampler = std::make_unique<ViewpointSampler<T>>(config_.center, config_.size / 2, 0.1);
            auto evaluator = std::make_unique<ViewpointEvaluator<T>>(*gpr, *cache, *acquisition, 10,
                                                                     config_.improvement_threshold);

            auto optimizer =
                    std::make_unique<ViewpointOptimizer<T>>(config_.center, config_.size, config_.min_size,
                                                            config_.max_iterations, *gpr, config_.size / 2, 0.1);

            auto initial_samples =
                    sampler->samplePoints(typename spatial::Octree<T>::Node(config_.center, config_.size));
            ...
            return {optimizer->getBestViewpoint(), optimizer->getBestViewpoint()
                                                           ? optimizer->getBestViewpoint()->getScore()
                                                           : std::numeric_limits<T>::lowest()};
        }

        [[nodiscard]] ViewPoint<T>
        selectInitialBest(const std::vector<ViewPoint<T>> &samples, const Image<> &target,
                          const std::shared_ptr<processing::image::ImageComparator> &comparator) const {
            auto best_sample = std::max_element(samples.begin(), samples.end(), [&](const auto &a, const auto &b) {
            ...
            LOG_INFO("Selected initial best viewpoint: {}, Score: {}", best_viewpoint.toString(), best_score);
