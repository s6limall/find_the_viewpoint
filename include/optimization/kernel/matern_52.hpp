...
namespace optimization::kernel {
    ...
    public:
        ...
        // calculates the kernel function for every pair of points in X and Y
            ...
            for (int i = 0; i < n; ++i) {
                for (int j = i; j < n; ++j) {
                    ...
                    if (i == j) {
                        ...
                        // Exploit symmetry to fill lower triangular part
                        K(j, i) = K(i, j);
                    }
                }
            }
...
#pragma omp parallel for
            for (int i = 0; i < n; ++i) {
                for (int j = i; j < n; ++j) {
                    ...
                    if (i != j) {
                        K_grad(j, i) = K_grad(i, j); // Leverage symmetry
                    }
                }
            }
