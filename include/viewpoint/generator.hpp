...
namespace viewpoint {
    ...
    public:
        explicit Generator(const double distance) :
            distance_(distance),
            halton_sampler_([this](const std::vector<double> &sample) { return transformer_.transform(sample); }),
    ...
    std::vector<ViewPoint<T>> Generator<T>::provision() {
        ...
        auto halton_sequence = halton_sampler_.generate(num_samples, {0.0, 0.0, 0.0}, {1.0, 1.0, 1.0});
