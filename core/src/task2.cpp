...
namespace task2 {
	...
	public:
		...
		void compute_pose_from_positon_and_object_center(Eigen::Vector3d positon, Eigen::Vector3d object_center) {
			...
			T(3, 3) = 1;
			Eigen::Vector3d Z;
			Z = object_center - positon;
			Z = Z.normalized();
			if ((Z - Eigen::Vector3d(0, 0, -1)).norm() < 1e-6) { Z = Eigen::Vector3d(1e-100, 1e-100, -1); }
			//to avoid -Z = (0,0,1) so there will be no X
			if ((Z - Eigen::Vector3d(0, 0, 1)).norm() < 1e-6) { Z = Eigen::Vector3d(1e-100, 1e-100, 1); }
			//to avoid Z = (0,0,1) so there will be no X
			Eigen::Vector3d X;
			X = (-Z).cross(Eigen::Vector3d(0, 0, 1));
			X = X.normalized(); //change is here from (0,1,0) to (0,0,1)
			Eigen::Vector3d Y;
			Y = X.cross(-Z);
			Y = Y.normalized();
	...
	//Preception
	/*class Perception : public core::Perception {
	public:
		...
		void render(View view, string image_save_path = "../rgb.png") {
			...
			cv::Mat img = core::Eye::render(view.pose_6d, test_image_save_path);
	...
	public:
		...
		// Constructor
		View_Planning_Simulator(View _target_view) {
		...
		cv::Mat render_view_image(View view) {
			...
			cv::Mat rendered_image = core::Eye::render(view.pose_6d, image_save_path);
		...
		bool test_view(View src_view, double max_score) {
			...
			core::Eye::render(src_view.pose_6d,
			                  "./task2/score/s_" + std::to_string(max_score) + "_dst_" + std::to_string(dis) + ".png");
		...
		View calculate_new_center(const View &A, const View &B, const View &C) {
			...
			View new_center;
			new_center.compute_pose_from_positon_and_object_center(centroid * 3.0,
			                                                       Eigen::Vector3d(1e-100, 1e-100, 1e-100));
		...
		// depth first search
		void dfs() {
			size_t view_num = cfg["dfs"]["num_corners"].as<size_t>();
			...
			for (size_t i = 0; i < view_num; ++i) {
				...
				search_views[i].compute_pose_from_positon_and_object_center(
					position.normalized() * 3.0, Eigen::Vector3d(1e-100, 1e-100, 1e-100));
			}

			// Create top view
			search_views[view_num].compute_pose_from_positon_and_object_center(
				Eigen::Vector3d(0.0, 0.0, 1.0) * 3.0, Eigen::Vector3d(1e-100, 1e-100, 1e-100));
		...
		double get_traversed_distance() {
			...
			for (int i = 1; i < selected_views.size(); i++) {
				pair<int, double> local_path = path_and_vis::get_local_path(
					selected_views[i - 1].pose_6d.block<3, 1>(0, 3), selected_views[i].pose_6d.block<3, 1>(0, 3),
					(Eigen::Vector3d(1e-10, 1e-10, 1e-10)).eval(), 1.0);
		...
		void show_view_image_path(string object_path, string pose_file_path, string rgb_file_path) {
			...
			viewer->setCameraPosition(0, 10.0 * sin(35.0 / 180.0 * acos(-1.0)), 10.0 * cos(35.0 / 180.0 * acos(-1.0)),
			                          object_center_world(0), object_center_world(1), object_center_world(2), 0, 0, 1);
			...
			for (int i = 0; i < chosen_views.size(); i++) {
				...
				Eigen::Vector3d LeftTop = path_and_vis::project_pixel_to_ray_end(
					0, 0, color_intrinsics, view_pose_world, line_length);
				Eigen::Vector3d RightTop = path_and_vis::project_pixel_to_ray_end(
					0, 720, color_intrinsics, view_pose_world, line_length);
				Eigen::Vector3d LeftBottom = path_and_vis::project_pixel_to_ray_end(
					1280, 0, color_intrinsics, view_pose_world, line_length);
				Eigen::Vector3d RightBottom = path_and_vis::project_pixel_to_ray_end(
					1280, 720, color_intrinsics, view_pose_world, line_length);
				...
				viewer->addLine<pcl::PointXYZ>(pcl::PointXYZ(O(0), O(1), O(2)), pcl::PointXYZ(LT(0), LT(1), LT(2)),
				                               view_color[0], view_color[1], view_color[2], "O-LT" + to_string(i));
				viewer->addLine<pcl::PointXYZ>(pcl::PointXYZ(O(0), O(1), O(2)), pcl::PointXYZ(RT(0), RT(1), RT(2)),
				                               view_color[0], view_color[1], view_color[2], "O-RT" + to_string(i));
				viewer->addLine<pcl::PointXYZ>(pcl::PointXYZ(O(0), O(1), O(2)), pcl::PointXYZ(LB(0), LB(1), LB(2)),
				                               view_color[0], view_color[1], view_color[2], "O-LB" + to_string(i));
				viewer->addLine<pcl::PointXYZ>(pcl::PointXYZ(O(0), O(1), O(2)), pcl::PointXYZ(RB(0), RB(1), RB(2)),
				                               view_color[0], view_color[1], view_color[2], "O-RB" + to_string(i));

				viewer->addLine<pcl::PointXYZ>(pcl::PointXYZ(LT(0), LT(1), LT(2)), pcl::PointXYZ(RT(0), RT(1), RT(2)),
				                               view_color[0], view_color[1], view_color[2], "LT-RT" + to_string(i));
				viewer->addLine<pcl::PointXYZ>(pcl::PointXYZ(LT(0), LT(1), LT(2)), pcl::PointXYZ(LB(0), LB(1), LB(2)),
				                               view_color[0], view_color[1], view_color[2], "LT-LB" + to_string(i));
				viewer->addLine<pcl::PointXYZ>(pcl::PointXYZ(RT(0), RT(1), RT(2)), pcl::PointXYZ(RB(0), RB(1), RB(2)),
				                               view_color[0], view_color[1], view_color[2], "RT-RB" + to_string(i));
				viewer->addLine<pcl::PointXYZ>(pcl::PointXYZ(LB(0), LB(1), LB(2)), pcl::PointXYZ(RB(0), RB(1), RB(2)),
				                               view_color[0], view_color[1], view_color[2], "LB-RB" + to_string(i));

				viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_LINE_WIDTH, 3,
				                                    "O-LT" + to_string(i));
				viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_LINE_WIDTH, 3,
				                                    "O-RT" + to_string(i));
				viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_LINE_WIDTH, 3,
				...
				if (is_show_path) {
					if (i != 0) {
						...
						vector<Eigen::Vector3d> points;
						int num_of_path = path_and_vis::get_trajectory_xyz(
							points, now_view_xyz, next_view_xyz, object_center_world, predicted_size, 0.2, 1.0);
						if (num_of_path == -1) {
						...
						viewer->addLine<pcl::PointXYZ>(pcl::PointXYZ(now_view_xyz(0), now_view_xyz(1), now_view_xyz(2)),
						                               pcl::PointXYZ(points[0](0), points[0](1), points[0](2)),
						                               path_color[0], path_color[1], path_color[2],
						                               "trajectory" + to_string(i) + to_string(i + 1) + to_string(-1));
						viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_LINE_WIDTH, 3,
						                                    "trajectory" + to_string(i) + to_string(i + 1) + to_string(
							                                    -1));
						for (int k = 0; k < points.size() - 1; k++) {
							viewer->addLine<pcl::PointXYZ>(pcl::PointXYZ(points[k](0), points[k](1), points[k](2)),
							                               pcl::PointXYZ(
								                               points[k + 1](0), points[k + 1](1), points[k + 1](2)),
							                               path_color[0], path_color[1], path_color[2],
							                               "trajectory" + to_string(i) + to_string(i + 1) +
							                               to_string(k));
							viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_LINE_WIDTH, 3,
							                                    "trajectory" + to_string(i) + to_string(i + 1) +
							                                    to_string(k));
				...
				if (is_show_image) {
					...
					for (int x = 0; x < image.cols; x += interval) {
						for (int y = 0; y < image.rows; y += interval) {
							Eigen::Vector3d pixel_end = path_and_vis::project_pixel_to_ray_end(
								x, y, color_intrinsics, view_pose_world, image_line_length);
					...
					viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 1,
					                                         "cloud_image" + to_string(chosen_views[i]));
				}
			}
			if (is_show_model) {
				/////////////////////////////////////////////////////////////////
	...
	void main(string object_name) {
		...
		for (int test_id = 0; test_id < test_num; test_id++) {
			...
			target_view.compute_pose_from_positon_and_object_center(
				Eigen::Vector3d(-0.879024, 0.427971, 0.210138).normalized() * 3.0,
				Eigen::Vector3d(1e-100, 1e-100, 1e-100));
			...
			View_Planning_Simulator view_planning_simulator(target_view);
			if (enable_dfs) {
				...
				//Eigen::Vector3d dst_view = Eigen::Vector3d(-0.879024, 0.427971, 0.210138).normalized() * 3.0;
				double dis = (target_view.pose_6d.block<3, 1>(0, 3) - view_planning_simulator.output_view.pose_6d.block<
					              3, 1>(0, 3)).norm();
				...
				if (enable_meta) {
					...
					fout_meta << "peak signal to noise ratio (PSNR):" << view_planning_simulator.calculate_PSNR() <<
							endl;
					fout_meta.close();
				}
				...
				if (enable_generate_imgs) {
					...
					for (int i = 0; i < view_planning_simulator.selected_views.size(); i++) {
						...
						core::Eye::render(view_planning_simulator.selected_views[i].pose_6d,
						                  rgb_file_path + "/rgb_" + to_string(i) + ".png");
					}
					fout_pose.close();
				}
				/*
				...
				*/
				core::Eye::render(view_planning_simulator.output_view.pose_6d,
				                  "./task2/selected_views/" + object_name + "/" + to_string(test_id) + "_final.png");
				core::Eye::render(target_view.pose_6d,
				                  "./task2/selected_views/" + object_name + "/" + to_string(test_id) + "_target.png");
			}
			if (enable_visulize) {
